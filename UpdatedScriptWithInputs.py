...
def process_record(record):
    ...
    try:
        ...
        print(f"Error processing file {content_name}: {e}")
...
try:
    ...
    FROM uuidb.ui_content
    WHERE content_name LIKE '{s3_folder_name}/%' AND content_type = 'CURRENT'
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        while True:
            ...
            if not rows:
                break
            executor.map(process_record, rows)
finally:
